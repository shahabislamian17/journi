...
const getApiUrl = () => {
  // Always use backend URL - both client and server
  const configuredUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000';
  // For server-side, replace localhost with 127.0.0.1 (some Node.js versions prefer this)
  if (typeof window === 'undefined') {
    return configuredUrl.replace('localhost', '127.0.0.1');
  }
  // For client-side, use the URL as-is
...
async function apiRequest(endpoint, options = {}) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[API Request]', { url, endpoint, apiUrl: API_URL });
  }
  ...
  try {
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      // If not JSON and response is not OK, it might be an HTML error page
      if (!response.ok) {
        // Check if it's HTML (Next.js error page)
        if (text.trim().startsWith('<!DOCTYPE') || text.trim().startsWith('<html')) {
          console.error('Received HTML instead of JSON. This usually means the API route is not matching correctly.');
          console.error('Request URL:', url);
          console.error('Response status:', response.status);
          console.error('Response text (first 500 chars):', text.substring(0, 500));
          throw new Error(`API route not found or error occurred. Status: ${response.status}`);
        }
        // Try to parse as JSON even if content-type doesn't say so
        try {
          const jsonData = JSON.parse(text);
          if (jsonData.error || jsonData.message) {
            console.error('API Error (from text):', jsonData);
            const error = new Error(jsonData.error || jsonData.message || `API request failed: ${response.status}`);
        ...
        throw new Error(`API request failed: ${text.substring(0, 500)}`);
      }
      return { data: text };
    }

    if (!response.ok) {
      // Extract error message from response - include more details
      let errorMessage = data?.error || data?.message || `API request failed with status ${response.status}`;
      
      // For 500 errors, include more details if available
      if (response.status === 500) {
        if (data?.message) {
          errorMessage = `${data.error || 'Server error'}: ${data.message}`;
        }
      ...
      // For 500 errors, try to extract more information
...
// Conversations API (for chat/messaging between users)
export const conversationsAPI = {
  getAll: (options = {}) => apiRequest('/api/conversations', options),
  getById: (id, options = {}) => apiRequest(`/api/conversations/${id}`, options),
  create: (data, options = {}) => apiRequest('/api/conversations', {
    method: 'POST',
    body: JSON.stringify(data),
    ...options,
  }),
  sendMessage: (conversationId, message, options = {}) => apiRequest(`/api/conversations/${conversationId}/messages`, {
    method: 'POST',
